...
interface UserContextType {
  user: UserProfileDetailsProps | null;
  setUser: (user: UserProfileDetailsProps | null) => void;
  logout: () => void;
}
...
export const UserContextCookieProvider: FC<UserContextProviderProps> = ({
  ...
  const [user, setUser] = useState<UserProfileDetailsProps | null>(() => {
  ...
  const encryptUserData = (data: UserProfileDetailsProps) => {
  ...
  const updateUser = (user: UserProfileDetailsProps | null) => {
    setUser(user);
    if (user) {
